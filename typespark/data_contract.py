...
def resolve_pk(t: type[TypedDataFrame]):
...
def foreign_key(t: Optional[type[TypedDataFrame]]):
    if t is None:
        return None
    pk = resolve_pk(t)
...
def field(cls: type[TypedDataFrame], f: attr.Attribute):
...
def serialize_class_metadata(cls: type[TypedDataFrame]):
...
def serialize_product(pkg: ModuleType):
    models = find_exposed_subclasses(pkg, TypedDataFrame)
